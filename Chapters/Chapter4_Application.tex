...
The scale is set to $0.33$ compromise between speed and matching robustness,
taking into account that compared to the computations performed on a computer
(see \autoref{ch:evaluation}) some detail is lost due to conversion between
iOS's \code{UIImage} format and OpenCV's \code{Mat} type.
...
here. The block passed to the operation uses its three arguments to update the
arrow visualisation once the computation has finished.
...
of this block is to drive some kind of visualisation. It then computes the
