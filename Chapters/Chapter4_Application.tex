...
\section{Objecttive-C \& C++ --- The PIMPL pattern}
...
The function then removes the image distortion on the
sparse set of keypoints instead of the whole image for efficiency\footnote{Since
   undistortion requires resampling of the image, it is better to find features
beforehand as some information may be lost by interpolation.}. On the
correspondences, RANSAC is used to find the best-fitting essential matrix. 
...
\begin{wrapfigure}{r}{.35\textwidth}
   \includegraphics[width=.3\textwidth]{gfx/image_data.pdf}
\end{wrapfigure}
...
\code{maxConcurrentOperationCount} property allows to set a maximum number of
