...
events in the app's life cycle. For example
\begin{itemize}
...
In order to respond to user input, the delegate pattern \citep[see]{gamma1995}
...
elaborate on the functions of the associated view controllers.
...
will modally present a \code{UIImagePickerController} and attach to it a
\code{RephotoManager} that controls the rephotography.
...
\newenvironment{mywrapfigure}[2]{
   \setlength\intextsep{0pt} % wrapfigure 'isn't a real environment' apparently; http://tex.stackexchange.com/a/266431/31250
   \wrapfigure#1#2
}
{
   \endwrapfigure
...
\begin{wrapfigure}{r}{.3\textwidth}
   \includegraphics[width=.25\textwidth]{gfx/image_data.pdf}
\end{wrapfigure}
