...
the \code{UIScrollViewDelegate} protocol (an equivalent of interfaces in other
...
define controllers and their relationships. While it is equally possible to
specifiy the presentation order programmatically, this allows for a cleaner
separation between user interface and business logic.  Storyboards consist of a
number of view controllers with associated views which are connected by
\code{Segue}s. When an application developed with storyboards starts up, a
\code{UINavigationController} is created which maintains a stack of view
controllers on which segues push new ones or pop old ones (\emph{unwind segue}).
The Storyboard for the application developed in this work is shown in
...
\nofloat{gfx/mainviewcontroller.PNG}{\code{MainViewController} view layout}
...
\nofloat{gfx/photochoosercontroller.PNG}{\code{PhotoChooserController} view
layout}
...
\nofloat{gfx/photochoosercontroller_photo.PNG}{\code{PhotoChooserController}
with loaded image}
...
\nofloat{gfx/settingsviewcontroller.PNG}{\code{SettingsViewController} view
layout}

\subsection{\code{RephotoManager}}

\hyphenation{Pose-Diff-er-ence-Es-ti-ma-tion-Op-er-at-ion}
...
\nofloat{gfx/rephoto_manager.PNG}{\code{RephotoManager} in
\code{HAS\_SECOND\_FRAME} state}
...
\nofloat{gfx/resultviewcontroller.PNG}{\code{ResultViewController} view layout}
...
\hyphenation{computePoseDifferenceBetweenCamera:-secondCamera:calibrationFile:scale:}
% dirty hack
...
public method \code{+[computePoseDifferenceBetweenCamera:secondCamera:calibrationFile:scale:]}.
