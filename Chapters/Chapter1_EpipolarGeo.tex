...
\begin{IEEEeqnarray*}{rClls}
   ...
   \mbf{x}$ \\
   \lambda_2\mbf{x}_2\widehat{T}\mbf{x}_2 & = &
   \mbf{x}_2\widehat{T}R\lambda_1\mbf{x}_1 + 0 & & $T \times T = 0$ \\
   \lambda_2\cdot 0 & = &
   \mbf{x}_2\widehat{T}R\lambda_1\mbf{x}_1 & & $\widehat{T}\mbf{x}_2$ is
   perpendicular to $\mbf{x}_2$ \\
   0 & = & \mbf{x}_2\underbrace{\widehat{T}R}_{E}\mbf{x}_1
...
complement of its preimage) $l_1 = e_1 \times \mbf{x}_2$ so that

\begin{equation} \label{eq:epiline}
   \forall\mbf{x}: \mbf{x}\in l_1 \Leftrightarrow \mbf{x}\cdot l_1 = 0.
\end{equation} 
...
with $E$ will yield a term $\mbf{x}_2E = l$ which fulfills 
\begin{equation}
   l \cdot \mbf{x}_1 = 0,
\end{equation}
...
user. The most prominent algorithms in this regard are the 8-Point algorithm due
to \citet{longuet-higgins1987} and the 5-Point algorithm proposed by
