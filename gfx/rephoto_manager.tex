\begin{tikzpicture}[very thick,every node/.append style={text
   ...
   % \makegrayinprint % :D this graphic just ignores the setting, contrarily to all others. #wtflatex

   \tikzset{>=Latex}
   \node[block] (start) {START};
   \node[block,below=of start] (first frame) {HAS\_FIRST \_FRAME};
   \node[decision,below=of first frame,
      % fill=RoyalBlue,
   ] (can match?) {First and second can be matched?};
   ...
   ;
   \draw[<-] (start) -- ++(0,1cm) node[above] {\emph{\rmfamily start}};
\end{tikzpicture}
